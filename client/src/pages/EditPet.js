...
const EditPet = () => {
  ...
  const fetchPet = async () => {
    try {
      ...
      setFormData({
        ...
        location: pet.location || { city: '', state: '', country: '' },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(error.response?.data?.message || error.response?.data?.errors?.[0]?.msg || 'Failed to update pet');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!isAdmin) {
    return (
      <div className="add-pet-page">
        <div className="container">
          <div className="login-prompt">
            <p>Only administrators can edit pets</p>
            ...
            <div className="form-group">
              <div className="special-needs-input">
                <input
                  ...
                  onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), handleAddSpecialNeed())}
                  placeholder="Enter special need"
                />
                <button type="button" onClick={handleAddSpecialNeed} className="btn btn-secondary">
                  Add
                </button>
              </div>
